...
class POLIAtrainer(Trainer):
    ...
    def _prepare_inputs(self, inputs: dict[str, Union[torch.Tensor, Any]], is_train=True) -> dict[str, Union[torch.Tensor, Any]]:
        ...
        total_completion_ids = torch.full((len(inputs), self.max_completion_length), self.processing_class.pad_token_id, device=device)
        ...
        while True:
            ...
            if "qwen" in self.model_id.lower():
                text = [
                    self.processing_class.apply_chat_template(inp["message"], tokenize=False, add_generation_prompt=True)
        ...
        has_gt_boxes = hasattr(inputs[0], "groundtruth_boxes") or "groundtruth_boxes" in inputs[0]
        has_bboxs = hasattr(inputs[0], "bboxs") or "bboxs" in inputs[0]

        if has_gt_boxes or has_bboxs:
            groundtruth_boxes = inputs[0].get("groundtruth_boxes", {})
            if not groundtruth_boxes and has_bboxs:
                bboxs_list = inputs[0].get("bboxs", [])
            ...
            for prompt_idx in range(len(completions_per_prompt)):
                ...
                for comp_idx, (completion, pred_bboxes) in enumerate(zip(prompt_completions, prompt_pred_bboxes)):
                    image_size = inputs[0].get("image_size", (224, 224)) if isinstance(inputs[0], dict) else (224, 224)
                    ...
                    if len(group_items) > 1:
                        ...
                        for gt_id, obj_rewards in per_object_rewards.items():
                            ...
                            for item_idx, comp_idx in enumerate(comp_indices):
                                global_idx = prompt_idx * self.num_generations + comp_idx
                                subgroup_advantages[global_idx] += self.subgroup_advantage_weight * obj_advantages[item_idx]
        ...
        if not matched_ids and (hasattr(inputs[0], "groundtruth_boxes") or "groundtruth_boxes" in inputs[0]):
            groundtruth_boxes = inputs[0].get("groundtruth_boxes", {})
    ...
    def compute_loss(self, model, inputs, return_outputs=False, num_items_in_batch=None):
        ...
        if hasattr(inputs, "matched_ids") and inputs["matched_ids"] is not None:
            matched_ids = inputs["matched_ids"]
            for i in range(len(matched_ids)):
                for j in range(len(matched_ids[i])):
                    if matched_ids[i][j] == "nomatch":
                        per_token_loss[i] *= 1.5
    ...
    def prediction_step(self, model, inputs, prediction_loss_only, ignore_keys: Optional[list[str]] = None):
        if not hasattr(self, "_eval_progress") and not self.is_train:
            self._eval_progress = {"total_samples": 0, "start_time": time.time(), "current_step": 0}
        ...
        if (not self.is_train) and self.max_eval_samples_per_dataset is not None and self.max_eval_samples_per_dataset > 0:
        ...
        if self.args.output_dir:
            ...
            for i in range(len(inputs["input_output_text"])):
                sample_data = {
                    ...
                    "raw_input": inputs["raw_inputs"][i] if i < len(inputs.get("raw_inputs", [])) else None,
                  
                    "image_path": (
                        os.path.basename(str(inputs["raw_inputs"][i]["image"]))
                        if i < len(inputs.get("raw_inputs", []))
                        and isinstance(inputs["raw_inputs"][i], dict)
                        and ("image" in inputs["raw_inputs"][i])
                        and inputs["raw_inputs"][i]["image"] is not None
                        else None
                    ),
                    "question": (
                        inputs["raw_inputs"][i].get("question")
                        if i < len(inputs.get("raw_inputs", [])) and isinstance(inputs["raw_inputs"][i], dict)
                        else None
                    ),
                    "gt_answer": (
                        inputs["raw_inputs"][i].get("gt_answer")
                        if i < len(inputs.get("raw_inputs", [])) and isinstance(inputs["raw_inputs"][i], dict)
                        else None
                    ),
                }
